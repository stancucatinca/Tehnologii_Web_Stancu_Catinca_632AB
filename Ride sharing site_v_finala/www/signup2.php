...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($parola !== $confirm_parola) {
        ...
        $sql = "INSERT INTO utilizatori (nume, email, telefon, parola, rol, nr_inmatriculare, culoare_masina) 
                VALUES ('$nume', '$email', '$telefon', '$parola', '$rol', '$nr_inmatriculare', '$culoare_masina')";

        try {
            if ($conn->query($sql) === TRUE) {
                // --- AICI ESTE SECRETUL: AUTENTIFICAREA AUTOMATĂ ---
                ...
                // 3. Acum contul4.php ne va primi, pentru că avem 'user_id' în sesiune
                echo "<script>alert('Cont creat cu succes!'); window.location.href='contul4.php';</script>";
            }
        } catch (mysqli_sql_exception $e) {
            // Verificăm dacă emailul e deja folosit (cod eroare 1062)
            if ($e->getCode() == 1062) {
